...
def generate_output(directory, objecto, flux_corrected=True, suffix=''):
    ...
    Inputs_optimal = glob.glob('%s/%s*%s*_single_opt.txt' %
                               (directory, objecto, suffix))
    Inputs_raw = glob.glob('%s/%s*%s*_single_raw.txt' %
                           (directory, objecto, suffix))
    Inputs_sigma = glob.glob('%s/%s*%s*_single_sig.txt' %
                             (directory, objecto, suffix))
...
def plot_object(directory, objecto, suffix=''):
    ...
    for i in range(len(optimal_files)):
        wavelength, optimal_flux, sigma = np.genfromtxt(
        ...
        plt.plot(wavelength, raw_flux / np.average(raw_flux),
                 linestyle='--', alpha=0.7, color='b', label='Raw Flux')
    ...
    if size == 1:
        ...
        for i in range(len(optimal_files)):
            wavelength_out, flux_out, sigma_out = np.genfromtxt(
                optimal_files[i], unpack=True)
            try:
                wavelength_array = np.vstack(
                    (wavelength_array, wavelength_out))
    ...
    np.savetxt('%s/%s_average%s.txt' %
               (directory, objecto, suffix), np.transpose(array))
...
def molly_parameter(directory, objecto, flux_corrected=True, suffix=''):
    ...
    for i in range(len(optimal_files)):
        ...
        molly_header = str(objecto) + " " + str(ut_out) + " " + str(date_out) + " " + str(ra_out) + " " + str(dec_out) + " " + str(
