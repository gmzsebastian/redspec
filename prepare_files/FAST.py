...
def prepare_data(file_directory = 'raw_data/*.fits', crop = False, rotate = False, flip = False, variables = ['DISPERSR', 'FILTER']):
    ...
    for i in range(len(Files)):
        ...
        #file_kind = File[0].header['DISPERSE']
        ...
        if file_type in ['zero', 'Bias', 'Zero', 'bias', 'BIAS', 'ZERO']:
            if len(glob.glob('bias')) == 0:
                os.system('mkdir bias')
            os.system('cp %s %s/%s_%s.fits'%(Files[i], 'bias', object_name, filename))
        if file_type in ['dark', 'Dark', 'DARK']:
            if len(glob.glob('dark')) == 0:
                os.system('mkdir dark')
            os.system('cp %s %s/%s_%s.fits'%(Files[i], 'dark', object_name, filename))
        elif file_type in ['COMP', 'AT2018hyz', 'G191B2B', 'OBJECT']:
        ...
        '''
        if file_type in ['zero', 'Bias', 'Zero', 'bias', 'object', 'Object', 'BIAS', 'COMP', 'AT2018hyz', 'G191B2B']:
            ...
            if crop:
                ...
                if file_type in ['zero', 'Bias', 'Zero', 'bias', 'BIAS']:
...
def example():
    ...
    extract_fits_info('raw_data/*.fits', ['OBJECT', 'EXPTYPE', 'EXPTIME', 'RA', 'DEC', 'DATE-OBS', 'TIME-OBS', 'DISPERSE'])

    # Prepare all the data into the right format, with the suffix names specified by variables
    prepare_data(variables = [''], rotate = False, crop = False)
